...
// IFarm is imported; no local minimal interface needed.
...
contract ProtocolCore is Ownable, ReentrancyGuard {
    ...
    function _distributeRevenue(uint256 farmId, uint256 netRevenue) internal {
        ...
        uint256 lpSplit = IFarm(farmDetail.farmAddress).lpIncentiveSplit();
        uint256 verifierSplit = IFarm(farmDetail.farmAddress).verifierIncentiveSplit();
        uint256 yodaSplit = IFarm(farmDetail.farmAddress).yieldYodaIncentiveSplit();
        uint256 ownerSplit = IFarm(farmDetail.farmAddress).farmOwnerIncentiveSplit();

        uint256 nonLpDen = verifierSplit + yodaSplit + ownerSplit; // expected = 100 - lpSplit
        require(nonLpDen > 0 && nonLpDen + lpSplit == 100, "Bad splits");

        uint256 verifierAmount = (netRevenue * verifierSplit) / nonLpDen;
        uint256 yieldYodaAmount = (netRevenue * yodaSplit) / nonLpDen;
        uint256 farmOwnerAmount = (netRevenue * ownerSplit) / nonLpDen;
