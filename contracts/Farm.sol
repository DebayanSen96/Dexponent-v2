...
contract Farm is Ownable, ReentrancyGuard {
    ...
    uint256 public immutable farmOwnerIncentiveSplit;
    ...
    struct Epoch { uint256 id; uint256 startTime; uint256 targetReserveBps; uint256 minReserve; uint256 deployedAtStart; uint256 harvestedInEpoch; }
    ...
    event ReserveParamsUpdated(uint256 reserveRatioBps, uint256 minReserve);
    event EpochStarted(uint256 id, uint256 targetReserveBps, uint256 minReserve);
    event RebalancedToTarget(uint256 targetReserve, uint256 available, uint256 deployedLiquidity);
    ...
    constructor(
        ...
        farmOwnerIncentiveSplit = _farmOwnerIncentiveSplit;
        require(
            _verifierIncentiveSplit + _yieldYodaIncentiveSplit + _lpIncentiveSplit + _farmOwnerIncentiveSplit == 100,
            "Farm: splits must sum to 100"
        );
        strategy = _strategy;
    ...
    function rebalanceStrategy() external onlyFarmOwner whenNotPaused {
        FarmStrategy(strategy).rebalance();
    }

    function rebalanceStrategy(bytes calldata data) external onlyFarmOwner whenNotPaused {
        FarmStrategy(strategy).rebalanceWithData(data);
    }

    function migrateStrategy(address newStrategy) external onlyFarmOwner {
        require(paused, "Farm: not paused");
        address old = strategy;
        if (old != address(0)) {
            FarmStrategy(old).emergencyWithdraw();
        }
        deployedLiquidity = 0;
    ...
    function setReserveRatioBps(uint256 _bps) external onlyFarmOwner {
        require(_bps <= 10000, "Farm: bps");
        reserveRatioBps = _bps;
        emit ReserveParamsUpdated(reserveRatioBps, minReserve);
    }

    function setMinReserve(uint256 _min) external onlyFarmOwner {
        minReserve = _min;
        emit ReserveParamsUpdated(reserveRatioBps, minReserve);
    }

    function startEpoch(uint256 targetReserveBps, uint256 _minReserve) external onlyFarmOwner whenNotPaused {
        require(targetReserveBps <= 10000, "Farm: bps");
        currentEpochId += 1;
        reserveRatioBps = targetReserveBps;
        minReserve = _minReserve;
        epochs[currentEpochId] = Epoch({ id: currentEpochId, startTime: block.timestamp, targetReserveBps: targetReserveBps, minReserve: _minReserve, deployedAtStart: deployedLiquidity, harvestedInEpoch: 0 });
        emit EpochStarted(currentEpochId, targetReserveBps, _minReserve);
    }

    function rebalanceToTarget() external onlyFarmOwner nonReentrant whenNotPaused {
        uint256 targetByRatio = (totalLiquidity * reserveRatioBps) / 10000;
        uint256 targetReserve = targetByRatio > minReserve ? targetByRatio : minReserve;
    ...
    function pullFarmRevenue() external virtual nonReentrant returns (uint256) {
        ...
        if (currentEpochId != 0) {
            epochs[currentEpochId].harvestedInEpoch += yieldInDXP;
        }
